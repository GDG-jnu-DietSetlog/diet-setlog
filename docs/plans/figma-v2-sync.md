...
| 친구 추가 | 기존 `FriendSearchScreen`은 검색/추천 친구 플로우 있음 | 홈2(`51:1240`)는 검색바, 카카오 친구 찾기, 추천 친구, 하단 CTA의 별도 고도화 시안 | app/friends | GitHub #79 |
...
| 피드 날짜별 변형 | 기존 날짜별 피드/스토리 다시보기 라우트 없음 | 피드(`56:1392`)는 날짜 앱바, `스토리 다시보기`, 나의 기록/친구 기록 필터, dashed 인증 ring | app/feed, app/routing | GitHub #78 |
...
2. 홈2 친구 추가 화면과 날짜별 스토리 다시보기 피드를 별도 이슈로 구현한다.
   - 홈2(`51:1240`)는 기존 `FriendSearchScreen`과 카카오 친구 찾기 API/딥링크 범위를 함께 검토한다.
   - 피드(`56:1392`)는 날짜별 record/feed API와 route 설계가 필요하다.
...
- 홈2 친구 추가 화면(`51:1240`)과 날짜별 스토리 다시보기 피드(`56:1392`)는 별도 구현 이슈로 남아 있다.
