...
### 작업 흐름
1. **이슈 먼저 작성** — 할 일을 이슈로 만들어 번호를 받는다(예: #12). 모든 작업은 이슈에서 출발한다.
2. 그 이슈번호로 브랜치 생성 — `feat/12-set-log-screen` (`main`/`develop`에서 최신 상태로 분기).
3. 작은 단위로 커밋 — 제목 끝에 `(#12)`를 붙인다. 예: `feat(app): 세트 기록 화면 추가 (#12)`.
4. PR 생성 — 제목 끝에 `(#12)`, 본문에 `Closes #12`. → 리뷰 → 승인 후 병합.
5. 병합은 **Squash and merge** 권장(히스토리를 깔끔하게).
6. 병합된 브랜치는 삭제(이슈는 `Closes #12`로 자동 종료).
...
- **header(`<type>(<scope>): <subject> (#이슈번호)`)는 필수**, body·footer는 선택.
- 한 줄 요약(subject)은 50자 이내 권장, 마침표(.) 없이.
- subject는 명령형 현재시제로: "추가한다"보다 "추가" / "add" 형태.
- **제목 끝에 관련 이슈번호를 `(#12)` 형태로 붙인다.** 작업은 항상 이슈를 먼저 만든 뒤 시작한다(아래 [작업 흐름](#작업-흐름) 참고).
...
### subject 작성
- 무엇을 했는지 간결하게 + 이슈번호. 예) `feat(app): 세트 기록 화면 추가 (#12)`
...
```
```
fix(server): 토큰 갱신 시 만료 검증 누락 수정 (#34)

Refs #34
```

> 제목의 `(#12)`는 GitHub에서 해당 이슈로 자동 링크된다. 이슈를 **닫고** 싶으면
> body/footer에 `Closes #12`를 함께 적는다(제목의 `(#12)`만으로는 닫히지 않음).
